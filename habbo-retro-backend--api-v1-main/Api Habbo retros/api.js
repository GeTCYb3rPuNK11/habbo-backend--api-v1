// Api made by Cyb3RPunK11 for open source purposes.

GoogleClientset = require ("http://localhost"); //  set your retro link without port
GoogleClientset.config  = server ("http://127.0.0.1") //  set your ip with port

GoogleClient = Client.extend({user: "", password:client})

Server.prototype. = client(serverinfo, client)

function serverinfo = url.parse(getserverinfo())

websocket.on = client.connect (addListener, "8080, 30000, connect")  // ad your game port of the emulator.
websocket.client = server (webserverInfo(),client) // dont touch this line
webserverInfo.addListener = websocket.shorthandListener; ("http:localhost:8080");
webserver.optimizeConnection = client.optimizeConnection; //

webserver.on("connection", function, optimizedConnection) {})
...
function() {
    ...
    window.FlashExternalInterface.logLoginStep = function(e, a) {
        ...
        window.HabboFlashClient.started = !0,
        n || "client.init.core.running" !== e || (n = !0,
        window.MainApp.postMessage({
            call: "hotel-ready"
        })),
...
function() {
    ...
    window.GameClientExternalInterface.disconnect = function() {
        window.MainApp.postMessage({
            call: "disconnect"
        })
    }
    ,
    window.GameClientExternalInterface.heartBeat = function() {
        window.MainApp.postMessage({
            call: "heartbeat"
        })
    }
    ,
    window.GameClientExternalInterface.loadConversionTrackingFrame = function() {
        var n = window.flashvars.unique_habbo_id;
        $("#conversion-tracking").attr("src", "/client/" + n + "/conversion-tracking")
    }
    ,
    window.GameClientExternalInterface.logout = function() {
        window.MainApp.postMessage({
            call: "logout"
        })
    }
    ,
    window.GameClientExternalInterface.openExternalPage = function(n) {
        window.MainApp.postMessage({
            call: "open-external",
            target: n
        })
    ...
    ,
    window.GameClientExternalInterface.openPage = function(n) {
        n = window.HabboPageTransformer.translate(n),
        window.HabboTracking.track("openpage", "", n),
        window.MainApp.postMessage({
            call: "open-page",
    ...
    ,
    window.GameClientExternalInterface.updateFigure = function(n) {
        window.MainApp.postMessage({
            call: "update-figure",
            target: n
        })
    ...
    ,
    window.GameClientExternalInterface.updateName = function(n) {
        window.MainApp.postMessage({
            call: "update-name",
            target: n
        })
    ...
    ,
    window.GameClientExternalInterface.openNews = function() {
        window.HabboTracking.track("news", "open", ""),
        window.MainApp.postMessage({
            call: "open-page",
            target: "/"
    ...
    ,
    window.GameClientExternalInterface.openAvatars = function() {
        window.GameClientExternalInterface.openPage("/settings/avatars")
    }
    ,
    window.GameClientExternalInterface.showInterstitial = function() {
        window.MainApp.postMessage({
            call: "show-interstitial"
        })
    }
    ,
    window.GameClientExternalInterface.subscriptionUpdated = function(n) {
        window.MainApp.postMessage({
            call: "update-habbo-club",
            target: n
        })
    ...
    ,
    window.GameClientExternalInterface.updateBuildersClub = function(n) {
        window.MainApp.postMessage({
            call: "update-builders-club",
            target: n
        })
...
function() {
    ...
    function n(n) {
        if (n.data) {
            var t = n.data;
            switch (console.log("Received event, with call: " + t.call),
            t.call) {
            case "open-link":
                e(t.target);
                break;
    ...
    window.HabboFlashClient = {
        ...
        init: function(n) {
            ...
            n || (console.error("Invalid FlashClient. Can't use JS->Flash interface."),
...
function() {
    ...
    window.NewUserReception.checkName = function(n) {
        ...
        window.HabboWebApi.checkName(n, function(e, a) {
            return e ? (console.error("Checking name failed!"),
            void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameFailed(n)) : void window.HabboFlashClient.flashInterface.newUserReceptionCheckNameResponse(n, a.code, a.validationResult || [], a.suggestions)
        })
    }
    ,
    window.NewUserReception.chooseRoom = function(n) {
        ...
        window.HabboWebApi.selectRoom(n, function(n) {
            return n ? (console.error("Choosing room failed!"),
            void window.HabboFlashClient.flashInterface.newUserReceptionChooseRoomFailed()) : void window.HabboFlashClient.flashInterface.newUserReceptionChooseRoomResponse()
        })
    }
    ,
    window.NewUserReception.claimName = function(n) {
        ...
        window.HabboWebApi.claimName(n, function(e, a) {
            return e ? (console.error("Claiming name failed!"),
            void window.HabboFlashClient.flashInterface.newUserReceptionClaimNameFailed(n)) : (window.HabboFlashClient.flashInterface.newUserReceptionClaimNameResponse(a.code, a.validationResult, a.suggestions),
            void ("OK" === a.code && window.MainApp.postMessage({
                call: "update-name",
                target: n
            })))
        })
    ...
    window.NewUserReception.saveOutfit = function(n, e) {
        ...
        window.HabboWebApi.saveFigure(n, e, function(n, a) {
            return n ? (console.error("Saving outfit failed!"),
            void window.HabboFlashClient.flashInterface.newUserReceptionSaveOutfitFailed()) : (window.HabboFlashClient.flashInterface.newUserReceptionSaveOutfitResponse(a.figureString, e, "OK"),
...
function() {
    ...
    l ? (r.supersonic_demo_campaigns && (w.demoCampaigns = 1),
    w.applicationUserId = r.supersonic_admin_id || r.account_id) : w.applicationUserId = r.account_id,
...
GoogleClientset = require (GoogleClientset);
Serverinfo = require (Server, addEventListener, "8080"); // change this to your port
Client = Client.extend = webserver.Client; = port; //
Websocket.url = location.protocol + "//" + location. = "localhost"
websocket.onopen = client.on = websocket.on (addEventListener);
HabboFlashClient = require(HabboFlashClient, "localhost/client") // put your /client
HabboFlashClient = client.on = HABBOFlashClient (server.url), "localhost/client"
Habboapi = require(Habboapi, "localhost/client")
Habboapi = client.on = HABBOapi (server.url, game.config);
HabboflashClient = client.on = HabboflashClient (OptimizeConnection, http.client)
HabboServer = client.on = HABBOServer ("localhost:8080") // put yout adress of game server
